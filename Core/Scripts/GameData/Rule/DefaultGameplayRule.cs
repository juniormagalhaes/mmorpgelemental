...
namespace MultiplayerARPG
{
    ...
    public partial class DefaultGameplayRule : BaseGameplayRule
    {
        ...
        public virtual float GetHitChance(BaseCharacterEntity attacker, BaseCharacterEntity damageReceiver)
        {
            ...
            if (attacker is BasePlayerCharacterEntity && damageReceiver is BaseMonsterCharacterEntity) {
                int levelDifference = Mathf.Abs(attackerLvl - dmgReceiverLvl);
                if (levelDifference >= 10 && attackerLvl < dmgReceiverLvl)
                {
